...
def _get_records(wosclient, wos_query, symbolicTimeSpan=None, count=5, offset=1):
    ...
    if wosclient.is_lite():
        ...
        return wosclient.search(wos_query, count, offset, None, symbolicTimeSpan).records

def prettify(xml):
    xml = _minidom.parseString(xml).toprettyxml(indent=' '*4)
    return '\n'.join([line for line in xml.split('\n') if line.strip()])

def single(wosclient, wos_query, symbolicTimeSpan=None, xml_query=None, count=5, offset=1):
    """Perform a single Web of Science query and then XML query the results."""
    records = _get_records(wosclient, wos_query, symbolicTimeSpan, count, offset)
...
def query(wosclient, wos_query, symbolicTimeSpan=None, xml_query=None, count=5, offset=1, limit=100):
    """Query Web of Science and XML query results with multiple requests."""
    results = [single(wosclient, wos_query, symbolicTimeSpan, xml_query, min(limit, count-x+1), x)
